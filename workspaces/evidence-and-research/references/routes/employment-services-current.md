...
- A published profile is valid for a maximum of six months at a time. If job search continues, it must be republished so that it remains published throughout the job search when the obligation applies. Job Market Finland currently reminds users to republish at least every six months.
...
Operator implication: from September onward, when work is relevant, ask whether job search is active, when it started, whether a post-1.9 `työnhakukeskustelu` has happened for an older job search, whether Työnhakuprofiili is published, when the current publication expires/was last republished, and what deadline/instruction appears in the person's own Työmarkkinatori account. Do not frame the profile as only an optional CV tip, and do not incorrectly threaten loss of benefit or job-search status.
