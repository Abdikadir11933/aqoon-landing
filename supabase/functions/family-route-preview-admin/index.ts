...
Deno.serve(async (request) => {
  ...
  const { data: lead, error: leadError } = await db.from("family_leads").select("city,main_need,sub_need,age_group,additional_needs").eq("id", leadId).maybeSingle();
  ...
  const domains = needDomainsForLead(lead, answers);
  if (!domains.length) return new Response(JSON.stringify({ preview_mode: "read_only", domains, candidates: [] }), { headers: responseHeaders });
  ...
  const now = Date.now(), city = String(lead.city || "").toLowerCase();
