...
function enhanceFunnel(){const rows=[...document.querySelectorAll('#fullFunnel .frow')];if(!rows.length)return;rows.forEach(r=>r.querySelectorAll('.drop-note').forEach(x=>x.remove()));const managed=rows.some(r=>r.dataset.funnelManaged==='1'),vals=rows.map(x=>Number(x.querySelector('strong')?.textContent)||0);rows.forEach((r,i)=>{if(i===0)return;const from=managed?Number(r.dataset.funnelBase)||0:vals[i-1],to=vals[i],lost=managed?Math.max(0,Number(r.dataset.funnelLost)||0):Math.max(0,from-to),rate=from?Math.round(lost/from*100):0;const d=document.createElement('div');d.className='drop-note '+(!lost?'neutral':'');d.textContent=lost?'-'+lost+' lost · '+rate+'%':'no loss';r.appendChild(d)});enhanceTraffic()}
...
function applyWorkContext(){const situation=document.querySelector('#questions [data-key="primary_situation"] .choice.on')?.dataset.value||'',intent=document.querySelector('#questions [data-key="work_intent"] .choice.on')?.dataset.value||'',student=situation==='Studying';document.querySelectorAll('#questions .match-extra[data-branch="student"]').forEach(x=>x.classList.toggle('hidden',!student));document.querySelectorAll('#questions .match-extra[data-branch="student-studying"]').forEach(x=>x.classList.toggle('hidden',!(student&&(intent==='Part-time'||intent==='Both'))));document.querySelectorAll('#questions .match-extra[data-branch="working"]').forEach(x=>x.classList.toggle('hidden',situation!=='Working'));document.querySelectorAll('#questions .match-extra[data-branch="jobseeker"]').forEach(x=>x.classList.toggle('hidden',situation!=='Unemployed / seeking work'))}
...
function enhance(id){try{const lead=(window.AqoonApp?.leads||[]).find(x=>x.id===id);if(!lead)return;C={lead,routes:routes(lead),programs:window.AqoonApp?.programs||[],interviews:window.AqoonApp?.interviews||[]};if(C.routes.includes('entrepreneurship'))['wanted_job','cv','cards','barrier','training'].forEach(k=>document.querySelector('#questions [data-key="'+k+'"]')?.closest('.question')?.remove());addFields(C.routes);bindExtras();$('dMeta').textContent=[lead.city,lead.main_need,lead.sub_need,'Interview topics: '+C.routes.join(' + ')].filter(Boolean).join(' · ');$('saveInterview').textContent=lead.interview_status==='completed'?'Save follow-up notes':'Save first interview & prepare research brief';$('saveInterview').onclick=save;collapseIfComplete(lead);renderCompleteness()}catch(e){$('err').textContent=e.message;$('err').classList.remove('hidden')}}
...
function password(){return sessionStorage.getItem('aqoon_tracker_password')||''}
async function api(url,body){const headers={'Content-Type':'application/json','x-tracker-password':password()},token=sessionStorage.getItem('aqoon_auth_token')||'';if(token)headers.Authorization='Bearer '+token;const r=await fetch(url,{method:'POST',headers,body:JSON.stringify(body),cache:'no-store'});let d={};try{d=await r.json()}catch{}if(!r.ok)throw Error(d.detail||d.error||'Request failed');return d}
...
const esc=v=>String(v==null?'':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
function pw(){return sessionStorage.getItem('aqoon_tracker_password')||''}
async function call(url,body){const headers={'Content-Type':'application/json','x-tracker-password':pw()},token=sessionStorage.getItem('aqoon_auth_token')||'';if(token)headers.Authorization='Bearer '+token;const r=await fetch(url,{method:'POST',headers,body:JSON.stringify(body),cache:'no-store'});let d={};try{d=await r.json()}catch{}if(!r.ok)throw Error(d.detail||d.error||'Request failed');return d}
