...
function renderCompleteness(){const host=$('questions');if(!host)return;const total=host.querySelectorAll('.match-extra[data-match-required="1"]:not(.hidden)').length;if(!total){$('matchCompleteness')?.remove();return}const answered=total-missing(collect()).length;let badge=$('matchCompleteness');if(!badge){badge=document.createElement('div');badge.id='matchCompleteness';badge.className='match-completeness';host.prepend(badge)}badge.textContent=answered+' / '+total+' matching fields answered';badge.classList.toggle('complete',answered===total)}
...
function enhance(id){try{const lead=(window.AqoonApp?.leads||[]).find(x=>x.id===id);if(!lead)return;C={lead,routes:routes(lead),programs:window.AqoonApp?.programs||[]};if(C.routes.includes('entrepreneurship'))['wanted_job','cv','cards','barrier','training'].forEach(k=>document.querySelector('#questions [data-key="'+k+'"]')?.closest('.question')?.remove());addFields(C.routes);bindExtras();$('dMeta').textContent=[lead.city,lead.main_need,lead.sub_need,'Interview topics: '+C.routes.join(' + ')].filter(Boolean).join(' · ');$('saveInterview').textContent='Save first interview & prepare research brief';$('saveInterview').onclick=save;renderCompleteness()}catch(e){$('err').textContent=e.message;$('err').classList.remove('hidden')}}
