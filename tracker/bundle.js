...
window.AqoonApp={get leads(){return leads},get partials(){return partials},updateLead:(id,patch)=>updateLead(id,patch)};
...
function openInterview(id){activeLead=leads.find(x=>x.id===id);if(!activeLead)return;answers={};window.AqoonInterview.activeLead=activeLead;window.AqoonInterview.currentAnswers=answers;$('dName').textContent='First interview · '+activeLead.name;$('dMeta').textContent=[activeLead.city,activeLead.main_need,activeLead.sub_need].filter(Boolean).join(' · ');$('iUrgency').value=activeLead.urgency||'normal';$('iNextAction').value=activeLead.latest_interview?.next_action||'';$('iFollow').value=activeLead.next_follow_up_at?new Date(new Date(activeLead.next_follow_up_at).getTime()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16):'';$('iNotes').value='';$('noteSuggestions')?.classList.add('hidden');$('promptWrap').classList.add('hidden');$('questions').innerHTML='';$('drawer').classList.remove('hidden')}
...
function addFields(rs){const host=$('questions'),existing=new Set([...host.querySelectorAll('[data-key]')].map(x=>x.dataset.key)),adult=rs.some(r=>['work','education','entrepreneurship','program','service_support','general'].includes(r)),sets=[...(adult?[F.core]:[]),...rs.map(r=>F[r]||F.general)];sets.flat().forEach(f=>{if(existing.has(f[0]))return;const d=document.createElement('div');d.className='question match-extra';d.dataset.matchRequired=f[4]?'1':'0';d.innerHTML='<label>'+f[1]+(f[4]?' <small style="color:#0A8F89">needed for matching</small>':'')+'</label>'+control(f);host.appendChild(d);existing.add(f[0])})}
...
function bindExtras(){document.querySelectorAll('#questions .match-extra .choice-row').forEach(r=>r.querySelectorAll('.choice').forEach(b=>b.onclick=()=>{if(r.classList.contains('match-multi'))b.classList.toggle('on');else{r.querySelectorAll('.choice').forEach(x=>x.classList.remove('on'));b.classList.add('on')}}))}
...
function missing(a){return[...document.querySelectorAll('#questions .match-extra[data-match-required="1"]')].filter(q=>{const k=q.querySelector('[data-key]')?.dataset.key,v=a[k];return !v||(Array.isArray(v)&&!v.length)})}
...
async function enhance(id){try{const[l,p]=await Promise.all([api({action:'list'}),api({action:'programs'})]),lead=(l.leads||[]).find(x=>x.id===id);if(!lead)return;C={lead,routes:routes(lead),programs:p.programs||[]};if(C.routes.includes('entrepreneurship'))['wanted_job','cv','cards','barrier','training'].forEach(k=>document.querySelector('#questions [data-key="'+k+'"]')?.closest('.question')?.remove());addFields(C.routes);bindExtras();$('dMeta').textContent=[lead.city,lead.main_need,lead.sub_need,'Interview topics: '+C.routes.join(' + ')].filter(Boolean).join(' · ');$('saveInterview').textContent='Save first interview & prepare research brief';$('saveInterview').onclick=save}catch(e){$('err').textContent=e.message;$('err').classList.remove('hidden')}}
async function save(){const a=collect(),miss=missing(a);if(miss.length){$('err').textContent='Complete matching fields first: '+miss.slice(0,4).map(x=>x.querySelector('label').textContent).join(' · ');$('err').classList.remove('hidden');miss[0].scrollIntoView({behavior:'smooth',block:'center'});return}const b=$('saveInterview');b.disabled=true;try{const pr=prompt(a),follow=$('iFollow').value?new Date($('iFollow').value).toISOString():null;await api({action:'save_interview',lead_id:C.lead.id,interview_type:C.routes.join('+'),answers:a,summary:'Routes: '+C.routes.join(', ')+'; matching criteria: '+Object.keys(a).length,research_prompt:pr,next_follow_up_at:follow,urgency:$('iUrgency').value,status:'completed'});$('promptBox').textContent=pr;$('promptWrap').classList.remove('hidden');$('err').textContent='';$('err').classList.add('hidden');setTimeout(()=>$('refresh')?.click(),300)}catch(e){$('err').textContent=e.message;$('err').classList.remove('hidden')}finally{b.disabled=false}}
