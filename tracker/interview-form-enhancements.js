...
function addEmploymentRuleChecks(root){
  const anchor=root.querySelector('.choice-row[data-key="jobseeker_active"]');if(!anchor||root.querySelector('[data-key="employment_deadline_missed"]'))return;
  const anchorQuestion=anchor.closest('.question');if(!anchorQuestion)return;
  const profileQuestion=root.querySelector('[data-key="job_search_profile"]')?.closest('.question');
  const deadline=document.createElement('div');deadline.className='question match-extra'+(anchorQuestion.classList.contains('hidden')?' hidden':'');deadline.dataset.branch=anchorQuestion.dataset.branch||'';deadline.dataset.branchSource=anchorQuestion.dataset.branchSource||'';
  deadline.innerHTML='<label>Any employment-service meeting, document or deadline missed?</label><div class="choice-row" data-key="employment_deadline_missed"><button type="button" class="choice" data-value="No">No</button><button type="button" class="choice" data-value="Yes">Yes</button><button type="button" class="choice" data-value="Not sure">Not sure</button></div><small class="muted" style="display:block;margin-top:8px">Since 1 Mar 2026 a first failure to deal with the employment authority as required can end the validity of job search. That is separate from unemployment-security sanctions. If Yes/Not sure, check whether job search still shows active in Asiointi and read the authority message; do not infer the consequence.</small>';
  (profileQuestion||anchorQuestion).insertAdjacentElement('afterend',deadline);wireChoiceRow(deadline.querySelector('.choice-row'));
  if(root.querySelector('[data-key="open_studies_status"]'))return;
  const studies=document.createElement('div');studies.className=deadline.className;studies.dataset.branch=deadline.dataset.branch||'';studies.dataset.branchSource=deadline.dataset.branchSource||'';
  studies.innerHTML='<label>Studying while jobseeking?</label><div class="choice-row" data-key="open_studies_status"><button type="button" class="choice" data-value="No studies">No studies</button><button type="button" class="choice" data-value="Open higher education">Open higher education</button><button type="button" class="choice" data-value="Other studies">Other studies</button><button type="button" class="choice" data-value="Not sure">Not sure</button></div><small class="muted" style="display:block;margin-top:8px">If studying, ask age, provider/type and start date. For jobseekers age 25+, covered open university/open UAS/kansalaisopisto/summer-university studies starting on or after 1 Jul 2026 do not affect unemployment security under the current rule, but normal job-search duties continue. Under-25 and other study cases still need the authority assessment.</small>';
  deadline.insertAdjacentElement('afterend',studies);wireChoiceRow(studies.querySelector('.choice-row'));
}
