...
function opportunitiesHtml(){const active=opportunities.filter(o=>!['not_interested','expired','closed'].includes(o.status));if(!active.length)return'';return '<section class="household-opportunities" id="householdOpportunities"><h4>Household & future opportunities</h4><p class="muted">Separate from today’s eligibility match. Confirm the need before acting.</p>'+active.map(o=>'<article class="household-opportunity"><strong>'+esc(o.signal_label||o.need_domain)+'</strong><small>'+esc(o.household_scope)+' · '+esc(o.status)+(o.earliest_contact_at?' · contact from '+esc(fmt(o.earliest_contact_at)):' · timing not set')+'</small><small>Contact permission: '+esc((o.contact_permission_status||'not_requested').replaceAll('_',' '))+'</small></article>').join('')+'</section>'}
function render(){
  const el=host();if(!el)return;
  const plan=activePlan(),otherPlans=plans.filter(p=>p!==plan);
  el.innerHTML='<h3>Case plan</h3>'+(plan?planCard(plan):'<p class="muted">After reviewing the research, give the agreed next route a short title to start the plan.</p><div class="new-plan-row"><input id="newPlanTitle" type="text" placeholder="e.g. Apply for private daycare voucher"><button type="button" id="newPlanBtn">Start case plan</button></div>')+(otherPlans.length?'<p class="muted" style="margin-top:9px">'+otherPlans.length+' earlier plan'+(otherPlans.length===1?'':'s')+' on this family.</p>':'')+opportunitiesHtml()+eventsHtml()+revisionsHtml()+'<p class="case-lifecycle-error hidden" id="caseLifecycleError"></p>';
...
async function load(){
  ...
  try{
    ...
    plans=lc.plans||[];events=lc.events||[];opportunities=lc.opportunities||[];revisions=hist.revisions||[];
    render();
...
window.AqoonCaseLifecycle={
  resolveActivePlan,
  reopenCase,
  contextForLead:id=>id===leadId?{plans:[...plans],events:[...events],opportunities:[...opportunities]}:{plans:[],events:[],opportunities:[]}
};
})();
