...
function syncBranches(){const kids=[...selected('household_children'),...selected('other_children_stages')],hasYoung=kids.includes('Under 3')||kids.includes('Age 3–6'),hasSchool=kids.includes('Grades 1–9'),work=selected('work_interest_gate')[0]||'',caregiver=selected('caregiver_future_goal'),oneOff=selected('work_search_scope')[0]==='One specific job / pilot / shift',close=selected('relationship_close_scope')[0]||'',evidence=!oneOff&&['Ask 3 quick access questions','Explore another need the family raised'].includes(close),explore=!oneOff&&close==='Explore another need the family raised';showGroup('relationship-gate',!oneOff);showGroup('baseline-evidence',evidence);showGroup('household-discovery',explore);showGroup('daycare',explore&&hasYoung);showGroup('other-child-daycare',explore&&hasYoung);showGroup('school',explore&&hasSchool);showGroup('vantaa-hobby',explore&&hasSchool&&city().toLowerCase()==='vantaa');showGroup('work-proof',explore&&(work==='Looking for work now'||work==='Likely within 12 months'||caregiver.some(v=>v==='Work now'||v==='Work within 12 months')));if(WEEKLY.enabled)showGroup('weekly',evidence&&(!WEEKLY.city||city().toLowerCase()===String(WEEKLY.city).toLowerCase()));const updates=document.getElementById('iRelevantUpdatesOk')?.closest('.question');if(updates){updates.classList.toggle('hidden',!explore);if(!explore)document.getElementById('iRelevantUpdatesOk').value='';}}
...
function section(){
  ...
  wrap.appendChild(head('Optional close','Finish after the active need, add three short access questions, or explore only a need the family already raised.','relationship-gate'));
  appendIfMissing(wrap,'relationship_close_scope',row('relationship_close_scope','Operator: how should this call close?',['Primary need only','Ask 3 quick access questions','Explore another need the family raised'],{note:'This is an operator control, not a family opinion or sales result.',group:'relationship-gate'}));
  wrap.appendChild(head('Three quick access questions','Use only when the call has room. These measure the access gap and never decide eligibility.','baseline-evidence'));
  if(!anyExists(['entry_service_awareness','prior_awareness']))wrap.appendChild(row('entry_service_awareness','Before AQOON, did they know the exact service/programme/opportunity discussed today existed?',['Yes – knew it','Had heard of it but did not understand it','No','Not sure'],{group:'baseline-evidence'}));
  if(!anyExists(['entry_service_self_navigation','self_navigation']))wrap.appendChild(row('entry_service_self_navigation','Without AQOON, would they have known what to do next?',['Yes','Partly / maybe','No','Not sure'],{group:'baseline-evidence'}));
  if(!anyExists(['entry_blockers','access_barriers']))wrap.appendChild(row('entry_blockers','What stopped them from acting before?',['Did not know it existed','Did not understand eligibility','Language','Form / digital system','Thought it cost too much','Did not know where to start','No one to ask','Childcare / timing','Transport','Trust / uncertainty','Other'],{multi:true,group:'baseline-evidence'}));
  wrap.appendChild(head('Household opportunity','Only continue when the family raised another need or welcomed the question.','household-discovery'));
  if(childCase()){
    appendIfMissing(wrap,'other_children_stages',row('other_children_stages','Are there other children in the household?',['No other children','Under 3','Age 3–6','Grades 1–9','Older children'],{multi:true,note:'Age bands only — do not record names here.',group:'household-discovery'}));
    appendIfMissing(wrap,'caregiver_future_goal',row('caregiver_future_goal','Does the parent / caregiver want help with their own next step?',['Work now','Work within 12 months','Finnish / education now','Finnish / education later','Start a business','No current goal','Not sure'],{multi:true,group:'household-discovery'}));
    appendIfMissing(wrap,'child_activity_interest',row('child_activity_interest','Could this child or another child want a hobby / activity?',['Yes – wants help now','Maybe later','No','Not sure'],{group:'household-discovery'}));
  }else{
    appendIfMissing(wrap,'household_children',row('household_children','Children in the household?',['No children','Under 3','Age 3–6','Grades 1–9','Older children'],{multi:true,note:'Age bands only. Do not assume a child identity.',group:'household-discovery'}));
    appendIfMissing(wrap,'work_interest_gate',row('work_interest_gate','Work situation for future support?',['Looking for work now','Likely within 12 months','Already working / no current need','Not looking for work now','Not sure'],{group:'household-discovery'}));
  }
  ...
  wrap.appendChild(head('Finish','Record only a confirmed additional need. Future hints stay separate from today’s matching.','household-discovery'));
  if(!anyExists(['cross_service_needs_all','other_needs_discovered']))wrap.appendChild(row('cross_service_needs_all','After talking, what else is genuinely relevant?',['Work','School / child support','Daycare','Children’s hobbies','Finnish / education','Kela / benefits','Programmes / training','Housing','Letters / applications','Nothing else now'],{multi:true,note:'Only tick a need the family confirmed. This does not add it to today’s route.',group:'household-discovery'}));
  appendIfMissing(wrap,'aqoon_return_intent',row('aqoon_return_intent','If another Finnish-system question comes up, would they contact AQOON again?',['Yes','Maybe','No','Not sure'],{group:'household-discovery'}));
  return wrap;
}
