...
function start(){document.addEventListener("click",e=>{const b=e.target.closest("[data-interview]");if(b){leadId=b.dataset.interview;setTimeout(restore,120)}if(e.target.closest("#structureNotes"))suggest();if(e.target.closest("#saveInterview"))setTimeout(()=>{if(!$("#promptWrap")?.classList.contains("hidden")){localStorage.removeItem(key());status("Saved to the family record")}},900)});$("#iNotes")?.addEventListener("input",saveDraft)}
