...
async function api(body){const r=await fetch(END,{method:'POST',headers:window.AqoonAuthHeaders(),body:JSON.stringify(body),cache:'no-store'}),d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.detail||d.error||'Could not load route preview');return d}
async function reviewApi(body){const r=await fetch(REVIEW,{method:'POST',headers:window.AqoonAuthHeaders(),body:JSON.stringify(body),cache:'no-store'}),d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.detail||d.error||'Could not save review');return d}
...
function render(data){const el=host();if(!el)return;lastCandidates=data.candidates||[];addMissing(lastCandidates);const cards=lastCandidates.map(c=>{const title=c.route_key.replace(/^route\./,'').replaceAll('.',' · '),status=c.match_status==='does_not_fit'?'Does not fit from known facts':c.match_status==='confirmed_match'?'Recorded criteria fit':'Possible — must confirm',miss=c.missing_fields?.length?'<p><strong>Ask next:</strong> '+c.missing_fields.map(k=>esc(label(k,c.criteria||[]))).join(' · ')+'</p>':'',confirm=c.confirmation_needed?.length?'<p><strong>Must confirm:</strong> '+c.confirmation_needed.map(esc).join(' · ')+'</p>':(!miss?'<p><strong>Next:</strong> operator reviews the recorded fit and chooses the route.</p>':''),conf=c.conflicting_criteria?.length?'<p class="route-conflict">'+c.conflicting_criteria.map(esc).join(' · ')+'</p>':'',steps=(c.steps||[]).slice(0,2).map(s=>'<li>'+esc(s)+'</li>').join(''),sources=(c.sources||[]).map(s=>'<a href="'+esc(s.url)+'" target="_blank" rel="noreferrer">'+esc(s.title||'Official source')+'</a>').join(' · '),disclosure=c.partner_disclosure_required?'<p class="route-disclosure">Partner/provider relationship must be disclosed before a referral.</p>':'';return '<article class="route-card"><div class="route-kicker">'+status+'</div><h3>'+esc(title)+'</h3>'+conf+miss+confirm+'<ol>'+steps+'</ol>'+disclosure+'<small>'+sources+'</small>'+reviewRow(c.route_key)+'</article>'}).join(''),gaps=(data.coverage_gaps||[]).length?'<div class="route-disclosure"><strong>Knowledge needs review:</strong> '+data.coverage_gaps.map(g=>esc(g.route_key)+': '+(g.reasons||[]).map(esc).join(' · ')).join('<br>')+'</div>':'';el.classList.remove('hidden');el.innerHTML='<div class="route-preview-head"><div><span>Current route preview</span><strong>Read-only — not an eligibility decision</strong></div><button type="button" class="btn secondary" id="refreshRoutePreview">Refresh</button></div>'+gaps+(cards||'<p class="muted">No current verified route is mapped to this need. Use focused research; do not guess.</p>');$('refreshRoutePreview')?.addEventListener('click',load);
